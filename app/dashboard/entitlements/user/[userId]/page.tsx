...
export default function UserEntitlementsPage() {
  ...
  const endpoint = useMemo(() => {
    return userId
      ? `/api/admin/entitlements-subscriptions/user/${encodeURIComponent(userId)}/entitlements`
      : "";
  }, [userId]);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        setError(process.env.NODE_ENV === "production" ? "Failed to load user entitlements" : e?.message || "Failed to load user entitlements");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 p-6 mb-8">
          <div className="flex items-start justify-between gap-6">
            ...
            <Link href="/dashboard/entitlements" className="text-blue-600 hover:underline">Back</Link>
          </div>
        </div>
                  ...
                  {items.length === 0 ? (
                    ...
                    items.map((e) => (
                      <tr key={e._id} className="hover:bg-blue-50/50 transition-all">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">{e.allocation || "-"}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">{(Number.isFinite(e.quantity as any) ? e.quantity : "∞") as any} / {e.consumed ?? 0}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">{e.createdAt ? new Date(e.createdAt).toLocaleString() : "-"}</td>
                      </tr>
                    ))
                  )}
                </tbody>
