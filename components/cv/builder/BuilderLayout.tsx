...
import { Eye, List, PlusIcon, Layout, Type } from "lucide-react";
import ResumeNav from "./ResumeNav";
import { TemplateLayout, TemplateColumn } from "@/types/cv/template";
...
export default function BuilderLayout({
  ...
  const handleSaveSection = useCallback(
    async (section: ResumeSection) => {
      ...
      try {
        // Personal Info section: Create CV + Draft
        if (section.type === "personal-info") {
          console.log("👤 Personal Info section - creating CV + draft...");
          if (onCreateCV) {
            const newCvId = await onCreateCV();
            console.log("✅ CV created successfully with ID:", newCvId);

            // Create initial draft immediately after CV creation
            if (onSaveDraft && newCvId) {
              console.log("📞 Creating initial draft...");
              await onSaveDraft(newCvId);
              console.log("✅ Initial draft created successfully");
            }
          }
        } else {
          // Other sections: Create/Update draft only
          console.log("📝 Other section - creating/updating draft...");
          if (onSaveDraft) {
            await onSaveDraft();
            console.log("✅ Draft updated successfully");
          } else {
            console.warn("⚠️ onSaveDraft is not available");
          }
        }
