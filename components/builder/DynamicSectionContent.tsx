...
import "@/lib/sections/initializeSections"; // Initialize section registry
...
export default function DynamicSectionContent({
  ...
  const getSectionHeading = (sectionType: string) => {
    if (templateConfig?.sectionHeadings?.[sectionType]) {
      return templateConfig.sectionHeadings[sectionType];
    }
  ...
  return (
    <div className="space-y-2">
      {availableSections.map((sectionType) => {
        ...
        const hasCustomHeading = templateConfig?.sectionHeadings?.[sectionType];
